...
func InsertMsg(database string, id string, content string, read uint, expire int64) (err error) {
	collection := global.App.Mongo.Database(database).Collection(id)
	comment := Trainer{
		Content:   content,
...
func FindMany(database string, sendId string, id string, time int64, pageSize int) (results []Result, err error) {
	...
	sendIdCollection := global.App.Mongo.Database(database).Collection(sendId)
	idCollection := global.App.Mongo.Database(database).Collection(id)
...
func FirsFindtMsg(database string, sendId string, id string) (results []Result, err error) {
	...
	var resultsYou []Trainer
	sendIdCollection := global.App.Mongo.Database(database).Collection(sendId)
	idCollection := global.App.Mongo.Database(database).Collection(sendId)
