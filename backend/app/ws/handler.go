...
func (c *Client) Read() {
	...
	for {
		...
		if sendMsg.Type == 1 {
			r1, _ := global.App.Redis.Get(context.Background(), c.ID).Result()
			r2, _ := global.App.Redis.Get(context.Background(), c.SendID).Result()
			if r1 >= "3" && r2 == "" { // 限制单聊
				replyMsg := ReplyMsg{
				...
				_, _ = global.App.Redis.Expire(context.Background(), c.ID, time.Hour*24*30).Result() // 防止重复骚扰，未建立连接刷新过期时间一个月
				continue
			} else {
				global.App.Redis.Incr(context.Background(), c.ID)
				_, _ = global.App.Redis.Expire(context.Background(), c.ID, time.Hour*24*30*3).Result() // 防止过快“分手”，建立连接三个月过期
			}
			...
			results, _ := FindMany(global.App.Config.Mongo.DbName, c.SendID, c.ID, int64(timeT), 10)
			if len(results) > 10 {
				results = results[:10]
			...
			results, err := FirsFindtMsg(global.App.Config.Mongo.DbName, c.SendID, c.ID)
			if err != nil {
				log.Println(err)
			}
			for _, result := range results {
